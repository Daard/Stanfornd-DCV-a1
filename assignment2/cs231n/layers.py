...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mu = np.mean(x, axis=0)
        # mu has (D,) shape, we need to transpose it
        xc = x - mu.T
...
def batchnorm_backward(dout, cache):
    ...
    if mode == "train":
        ...
        dx_norm = gamma * dout
        dvar = -0.5 * np.sum(dx_norm * xc, axis=0) * (std ** -3)
        dxc = dx_norm * (std ** -1) + 2 / N * dvar * xc
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1. / N) * ((var + eps) ** -.5) * (N * dx_norm - np.sum(dx_norm, axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape
    F, C, H1, W1 = w.shape
    new_size = lambda n, f: int(1 + (n - f + 2 * pad) / stride)
    H2 = new_size(H, H1)
    W2 = new_size(W, W1)
...
def max_pool_forward_naive(x, pool_param):
    ...
    def new_size(size, filter_size):
        return int((size - filter_size) / S + 1)
    H2 = new_size(H1, FH)
    W2 = new_size(W1, FW)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(H2):
                for w in range(W2):
                    filtered = x[n, c, h*S:h*S + FH,w*S:w*S + FW]
                    zeros = filtered == np.max(filtered)
                    dx[n, c, h*S:h*S + FH,w*S:w*S + FH] = dout[n, c, h,  w] * zeros
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
