...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            X_tmp, cache['affine%d' % i] = affine_forward(X_tmp, self.params['W%d' % i], self.params['b%d' % i])
            if self.normalization == "batchnorm":
                X_tmp, cache['batch%d' % i] = batchnorm_forward(X_tmp, self.params['gamma%d' % i],
                                                                self.params['beta%d' % i],
                                                                self.bn_params[i - 1])
            X_tmp, cache['relu%d' % i] = relu_forward(X_tmp)
            if self.use_dropout:
                X_tmp, cache['dropout%d' % i] = dropout_forward(X_tmp, self.dropout_param)

        WLast, bLast = 'W%d' % self.num_layers, 'b%d' % self.num_layers
        scores, cache_scores = affine_forward(X_tmp, self.params[WLast], self.params[bLast])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        for i in reversed(range(1, self.num_layers)):
            if self.use_dropout:
                dx = dropout_backward(dx, cache['dropout%d' % i])
            dx = relu_backward(dx, cache['relu%d' % i])
            if self.normalization == "batchnorm":
                dx, grads['gamma%d' % (i)], grads['beta%d' % (i)] = batchnorm_backward(dx, cache['batch%d' % i])
            dx, grads['W%d'%i], grads['b%d'%i] = affine_backward(dx, cache['affine%d' % i])
